...
import { MapStatsResp, DrawMaterial, UserInfo, PrintEvent, ProcessType, WorldDataSet, DrawContext } from "./type";
 ...
 * 绘制指定世界地图
 */
export const drawWorld = async function (dataSet: WorldDataSet, context: DrawContext) {
 ...
 */
const materialCreatorFactory = function (context: DrawContext, roomStats: MapStatsResp) {
 ...
 * @param context 上下文
 */
const drawAllRoom = async function (dataSet: WorldDataSet, context: DrawContext) {
    ...
    const roomDrawer = async (material: DrawMaterial) => {
    ...
    // 地图行绘制器
    const rowDrawer = async (materialRow: DrawMaterial[]) => {
        // 绘制该行的房间
